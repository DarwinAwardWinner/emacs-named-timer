...
only. If non-nil, the cancelled timer is not removed from
`named-timer-table'. This is used for efficiency in cases where
that hash table entry is about to be replaced anyway, so deleting
it is pointless."
